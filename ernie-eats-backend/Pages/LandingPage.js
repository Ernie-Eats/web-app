...
import * as RestaurantPageDatabase from '../Database/ResturantPageDatabase.js';
    ...
    if (events.success) {
        ...
        events.model.forEach(event => {
            ...
            if (eventTime > 0 && eventTime < 7) {
                RestaurantDatabase.findAllResturants().then(restaurants => {
                    if (restaurants.success) {
                        ...
                        if (foundRestaurant !== undefined) {
                            ...
                            eventElm.setAttribute("resturant-name", foundRestaurant.name);
...
await ReviewDatabase.findAllReviews().then(reviews => {
    if (reviews.success) {
        reviews.model.forEach(review => restaurantMap.set(review.resturantId,
            restaurantMap.has(review.resturantId)
                ? +(restaurantMap.get(review.resturantId)) + +(review.rating)
                : +(review.rating))
        );
    }
});
    ...
    if (restaurants.success) {
        ...
        topThree.forEach((pair, index) => {
            ...
            if (foundRestaurant !== undefined) {
                ...
                info.appendChild(document.createElement("p")).innerText = ""; // Need to get the RestaurantPageDatabase updated.
            }
        });
    }
});
